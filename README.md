...
flowchart TD
    ...
    G --> H["parse NDJSON to containers array"]
    H --> I["push into S and H state objects"]
    I --> J["buildAlerts - check 6 thresholds"]

    J --> K["renderSidebar and renderMiniStats"]
    K --> L["tickCharts - update Chart.js"]
    L --> M{"Active panel?"}

    M -- Overview --> N["metric cards, sparklines, suggestions"]
    M -- Live Charts --> O["4 line charts and fleet bar"]
    M -- Containers --> P["progress bars and limits modal"]
    M -- Alerts --> Q["alert list and rules table"]
    M -- Prediction --> R["OLS and EMA charts and history table"]
    M -- Health --> S["tile grid and lifecycle events"]
    M -- Heatmap --> T["time vs container CPU grid"]

...
function chartOpts(yMax){
  return {responsive:true,maintainAspectRatio:false,animation:false,
    ...
    plugins:{legend:{display:false},tooltip:{backgroundColor:'#13161b',borderColor:'#2a3140',borderWidth:1,titleColor:'#8a95a3',bodyColor:'#e8ecf0',titleFont:{family:"'Space Mono',monospace",size:10},bodyFont:{family:"'Space Mono',monospace",size:11},padding:8}},
    scales:{x:{ticks:{color:'#4a5568',font:{family:"'Space Mono',monospace",size:8},maxTicksLimit:8,maxRotation:0},grid:{color:'rgba(31,37,46,.8)'}},
            y:{min:0,max:yMax,ticks:{color:'#4a5568',font:{family:"'Space Mono',monospace",size:8},maxTicksLimit:5},grid:{color:'rgba(31,37,46,.8)'}}}};
}

function mkLine(id,color,yMax){
  const el=document.getElementById(id); if(!el) return null;
  return new Chart(el,{type:'line',data:{labels:[],datasets:[{data:[],borderColor:color,backgroundColor:color+'18',borderWidth:2,pointRadius:0,fill:true,tension:0.35}]},options:chartOpts(yMax)});
}

function initCharts(){
  [cCpu,cMem,cNet,cDisk,cFleet].forEach(c=>c&&c.destroy());
  cCpu=mkLine('chart-cpu','#00d4ff',100);
  cMem=mkLine('chart-mem','#00ff88',100);
  cNet=mkLine('chart-net','#ff6b35',120);
  cDisk=mkLine('chart-disk','#ffd700',90);
...
function pushCharts(d,doUpdate=true){
  ...
  function push(ch,val){if(!ch)return;ch.data.labels.push(lbl);ch.data.datasets[0].data.push(Math.round(val*10)/10);
    while(ch.data.labels.length>win){ch.data.labels.shift();ch.data.datasets[0].data.shift();}
    if(doUpdate) ch.update('none');}
  push(cCpu,d.cpu);push(cMem,d.mem);push(cNet,d.net);push(cDisk,d.dk);
}
...
function tickCharts(){
  ...
  if(cFleet){
    ...
    cFleet.data.datasets[0].backgroundColor=CC.slice(0,liveContainers.length).map(c=>c+'88');
    cFleet.data.datasets[0].borderColor=CC.slice(0,liveContainers.length);
    cFleet.update('none');
  }
  updateChartVals();
}
